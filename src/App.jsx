    ...
    <div className="screen-ui">
      ...
      <main className="workspace">
        ...
        <section className="card"><div className="section-head"><span className="number">1</span><div><h2>Пациент</h2><p>Основные сведения из пунктов 1–8 оригинального бланка.</p></div></div><div className="grid three">
          ...
          <Field label="Дата рождения" name="birthDate" value={form.birthDate} onChange={update} type="date"/><Field label="Возраст, лет" name="ageYears" value={form.ageYears} onChange={update} type="number"/><Field label="Возраст, месяцев" name="ageMonths" value={form.ageMonths} onChange={update} type="number"/>
        ...
        <section className="card"><div className="section-head"><span className="number">2</span><div><h2>Состояние и помощь</h2><p>Диагноз, обстоятельства, медицинская помощь и дополнительные показатели.</p></div></div>
          ...
          <div className="grid two"><TextField label="Диагноз врача (фельдшера) бригады СМП" name="diagnosis" value={form.diagnosis} onChange={update} rows={4}/><TextField label="Обстоятельства заболевания / несчастного случая" name="circumstances" value={form.circumstances} onChange={update} rows={4}/><TextField label="Оказанная медицинская помощь" name="help" value={form.help} onChange={update} rows={5}/><Field label="Время вызова / прибытия" name="callTime" value={form.callTime} onChange={update} type="time"/></div>
        </section>
